...
function buildFallbackResponse(message, user, context = {}) {
  ...
  const ctx = normalizeContext(context);
  const topic = extractTopic(message);
  const fallbackVariant = simpleHash(message) % 3;
  ...
  if (goals) {
    return {
      reply:
        fallbackVariant === 0
          ? `For "${goals}", run one focused sprint now and one deadline task tonight.`
          : fallbackVariant === 1
            ? `To move "${goals}" forward, complete one high-priority block now, then one application/deadline step later today.`
            : `Progress "${goals}" with two steps: one 25-minute sprint now and one deadline-linked action tonight.`,
      actions: [
        {
          type: "task",
  ...
  if (interests.length > 0) {
    return {
      reply:
        fallbackVariant === 0
          ? `Use your ${interests.join(", ")} interests for one skill task and one opportunity action today.`
          : fallbackVariant === 1
            ? `Your "${topic}" question fits your ${interests.join(", ")} path. Do one practice task and one opportunity move today.`
            : `For "${topic}", leverage ${interests.join(", ")}: one concrete build task plus one opportunity action today.`,
  ...
  return {
    reply:
      fallbackVariant === 0
        ? `For "${topic}", do one 25-minute focused block now and one follow-up step tonight, ${name}.`
        : fallbackVariant === 1
          ? `Your next best move on "${topic}" is one quick execution sprint now, then one deadline-based action before bed.`
          : `Let’s execute "${topic}" in two steps: a focused sprint now and one measurable result tonight.`,
