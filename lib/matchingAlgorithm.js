...
export function matchOpportunities(user, opportunities) {
  ...
  const majorTags = Array.isArray(majorTrack?.opportunityTags)
    ? majorTrack.opportunityTags.map(normalizeTag).filter(Boolean)
    : [];
  ...
  return opportunities
    .map((opportunity) => {
      ...
      const majorMatchingTags = opportunityTags.filter((tag) => majorTags.includes(tag));
      ...
      const scoreFromMajor = Math.min(4, majorMatchingTags.length);
      const scoreFromDeadline = deadlineSoon ? 1 : 0;
      const matchScore = scoreFromTags + scoreFromMajor + scoreFromDeadline;
      ...
      } else if (majorMatchingTags.length >= 2) {
        recommendedAction = "Major Aligned";
      }
