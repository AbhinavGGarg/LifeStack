...
export async function updateUserProfile(userId, profileUpdates) {
  ...
  const normalizedProfile = {
    ...
    activityHours:
      ...
      Number.isFinite(profileUpdates.activityHours)
        ...
        : null,
    extracurriculars: Array.isArray(profileUpdates?.extracurriculars)
      ? profileUpdates.extracurriculars
          .map((item) => String(item).trim())
          .filter(Boolean)
      : [],
    intendedMajor: String(profileUpdates?.intendedMajor || "").trim(),
    targetRole: String(profileUpdates?.targetRole || "").trim(),
    onboardingComplete:
  ...
  if (hasFirebaseAdminConfig()) {
    ...
    const nextProfile = {
      ...
      extracurriculars: normalizedProfile.extracurriculars,
      intendedMajor: normalizedProfile.intendedMajor,
      targetRole: normalizedProfile.targetRole,
      onboardingComplete:
