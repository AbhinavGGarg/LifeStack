...
function toUserFromDoc(doc) {
  ...
  return {
    ...
    profile: {
      ...
      majorRecommendation: data?.profile?.majorRecommendation || "",
...
export async function createUser({ email, passwordHash, profile }) {
  ...
  const user = {
    ...
    profile: {
      ...
      majorRecommendation: String(profile?.majorRecommendation || "").trim(),
...
export async function updateUserProfile(userId, profileUpdates) {
  ...
  const normalizedProfile = {
    ...
    intendedMajor: hasIntendedMajor
      ...
      : undefined,
    majorRecommendation: hasMajorRecommendation
      ? String(profileUpdates?.majorRecommendation || "").trim()
      : undefined,
  ...
  if (hasFirebaseAdminConfig()) {
    ...
    const nextProfile = {
      ...
      majorRecommendation: hasMajorRecommendation
        ? normalizedProfile.majorRecommendation
        : existing.profile.majorRecommendation || "",
  ...
  users[index] = {
    ...
    profile: {
      ...
      majorRecommendation: hasMajorRecommendation
        ? normalizedProfile.majorRecommendation
        : users[index]?.profile?.majorRecommendation || "",
