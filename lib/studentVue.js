...
function extractSoapResultWithRegex(soapText) {
  const match = String(soapText || "").match(
    /<(?:\w+:)?ProcessWebServiceRequestResult\b[^>]*>([\s\S]*?)<\/(?:\w+:)?ProcessWebServiceRequestResult>/i
  );

  return match?.[1] ? String(match[1]).trim() : null;
}

function extractEncodedSoapResult(soapText) {
  const regexResult = extractSoapResultWithRegex(soapText);
  if (regexResult) {
    return regexResult;
  }

  try {
    const parsedSoap = createSoapParser().parse(soapText);
    const parsedResult = findResultNode(parsedSoap);
...
export async function fetchStudentVueGrades({
  ...
  try {
    ...
    response = await fetch(resolvedServiceUrl, {
      method: "POST",
      headers: {
    ...
    if (error && typeof error === "object" && "name" in error && error.name === "AbortError") {
      throw new Error("StudentVUE request timed out. Try again in a few seconds.");
    }
    throw new Error("Unable to reach StudentVUE right now. Please try again.");
  }
  ...
  const encodedResult = extractEncodedSoapResult(soapText);

  if (typeof encodedResult !== "string") {
    throw new Error("Unable to parse portal response. Please verify StudentVUE portal URL.");
  }
