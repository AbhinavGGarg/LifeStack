...
function gpaScoreWithSignals(gpa, benchmark, academicSignals) {
  const baseline = gpaScore(gpa, benchmark);
  const courseCount = Number(academicSignals?.courseCount) || 0;
  const hasGrades = Boolean(academicSignals?.hasGrades);
  const gradeVariance = Number(academicSignals?.gradeVariance);
  ...
  if (hasGrades) {
    ...
    if (Number.isFinite(gradeVariance)) {
      if (gradeVariance <= 4) {
        adjusted += 2;
      } else if (gradeVariance >= 12) {
        adjusted -= 4;
      }
    }
  }

  return Math.round(clamp(adjusted, 10, 100));
}
...
function suggestionsFromBreakdown(breakdown, benchmark, academicSignals) {
  const suggestions = [];
  ...
  if (!academicSignals?.hasGrades) {
    suggestions.push("Add your grade link and import courses in Grades to improve estimate accuracy.");
  }
...
function confidenceFromData(profile, academicSignals) {
  let score = 0;

  if (typeof profile?.gpa === "number" && Number.isFinite(profile.gpa)) {
    score += 35;
  }
  if (academicSignals?.hasGrades) {
    score += 35;
  }
  if (academicSignals?.hasPortal) {
    score += 15;
  }
  if ((Number(academicSignals?.courseCount) || 0) >= 5) {
    score += 10;
  }
  if (typeof profile?.activityHours === "number" && Number.isFinite(profile.activityHours)) {
    score += 5;
  }
  ...
  const level = normalized >= 80 ? "High" : normalized >= 55 ? "Medium" : "Low";
...
export function estimateCollegeFit(profile, college, options = {}) {
  const academicSignals = options?.academicSignals || profile?.academicSignals || null;
  const extracurricularCount = Array.isArray(profile?.extracurriculars)
    ? profile.extracurriculars.length
    : 0;

  const breakdown = {
    gpa: gpaScoreWithSignals(profile?.gpa, college, academicSignals),
  ...
  const dataConfidence = confidenceFromData(profile, academicSignals);
  ...
  return {
    ...
    breakdown,
    suggestions: suggestionsFromBreakdown(breakdown, college, academicSignals),
    dataConfidence,
  };
}

export function recommendCollegeMatches(profile, limit = 5, options = {}) {
  return collegeBenchmarks
    .map((college) => estimateCollegeFit(profile, college, options))
    .sort((a, b) => {
