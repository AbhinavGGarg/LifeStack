import { getMajorTrack, getMajorLabel, resolveMajor } from "@/lib/majorGuidance";
...
function buildFallbackRecommendation(user, opportunity) {
  ...
  const resolvedMajor = resolveMajor(user?.intendedMajor, user?.majorRecommendation);
  const majorTrack = getMajorTrack(user?.intendedMajor, user?.majorRecommendation);
  const majorTags = Array.isArray(majorTrack?.opportunityTags)
    ? majorTrack.opportunityTags.map((item) => String(item).toLowerCase())
    : [];
  const majorMatched = tags.filter((tag) => majorTags.includes(tag));

  if (majorMatched.length > 0) {
    return `This fits your ${getMajorLabel(resolvedMajor)} path through ${majorMatched.slice(0, 2).join(" and ")} experience.`;
  }
...
export async function getOpportunityRecommendation(user, opportunity) {
  ...
  const resolvedMajor = resolveMajor(user?.intendedMajor, user?.majorRecommendation);
  const resolvedMajorLabel = getMajorLabel(resolvedMajor);
  ...
  const prompt = [
    ...
    `Student typed major: ${user?.intendedMajor || "Not set"}`,
    `Resolved major path: ${resolvedMajorLabel}`,
